...
function CodeReview({ taskId, socket, projectId, projectTasks = [] }) {
  ...
  return (
    <div className="mt-3">
        ...
        <div className="d-flex gap-2">
          {activeSnippet && (
            <button
              ...
              onClick={async () => {
                setAiReviewing(true); setAiResult(null);
                try {
                  const res = await api.post('/ai/code-review', { snippet_id: activeSnippet.id });
                  setAiResult(res.data);
                  fetchComments(activeSnippet.id);
                } catch (err) { setAiResult({ summary: 'AI review failed', score: 0 }); }
                finally { setAiReviewing(false); }
              }}
              disabled={aiReviewing}
            >
              {aiReviewing ? '⏳ Reviewing...' : '🤖 Review with AI'}
            </button>
          )}
          <button
      ...
      {/* AI Review Result Banner */}
      {aiResult && (
        <div className={`alert ${aiResult.score >= 75 ? 'alert-success' : aiResult.score >= 50 ? 'alert-warning' : 'alert-danger'} py-2 d-flex justify-content-between align-items-center`}>
          <div>
            <strong>🤖 AI Score: {aiResult.score}/100</strong>
            <span className="ms-2">{aiResult.summary}</span>
            {aiResult.comments_added > 0 && <span className="badge bg-dark ms-2">{aiResult.comments_added} comments added</span>}
          </div>
          <button className="btn-close" onClick={() => setAiResult(null)} />
        </div>
      )}
